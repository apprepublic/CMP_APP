    ...
    <div className="space-y-gutter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        ...
        <div className="lg:col-span-4 space-y-gutter">
          ...
          <div className="bg-primary-container rounded-xl p-6 relative overflow-hidden shadow-lg border border-primary-fixed-dim/20">
            ...
            <div className="bg-primary p-4 rounded-lg flex items-center justify-between border border-secondary-fixed/30 relative z-10 group">
              <span className="font-data-lg text-data-lg text-secondary-fixed tracking-wider select-all">CMP-ALEX99</span>
              <button onClick={copyCode} className="text-secondary-fixed hover:text-secondary-fixed-dim transition-colors bg-secondary-fixed/10 p-2 rounded-md">
                {copied ? (
                  <span className="material-symbols-outlined text-success-verified">check</span>
                ) : (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-surface-alt rounded-xl p-5 shadow-sm border border-outline-variant/30 flex flex-col justify-between">
            ...
            <div className="bg-surface-alt rounded-xl p-5 shadow-sm border border-outline-variant/30 flex flex-col justify-between">
              <div className="flex items-center gap-2 text-success-verified mb-3">
            ...
            <div className="col-span-2 bg-surface-alt rounded-xl p-5 shadow-sm border-2 border-secondary-container/50 relative overflow-hidden">
              <div className="absolute right-0 bottom-0 opacity-10 transform translate-x-1/4 translate-y-1/4">
                <span className="material-symbols-outlined text-[120px]">savings</span>
              </div>
              <div className="flex items-center gap-2 text-secondary-container mb-2 relative z-10">
              ...
              <div className="font-body-sm text-body-sm text-success-verified mt-2 flex items-center gap-1 relative z-10">
        ...
        <div className="lg:col-span-8 space-y-gutter">
          {/* Chart Card */}
          <div className="bg-surface-alt rounded-xl p-6 shadow-sm border border-outline-variant/30">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-h3 text-h3 text-primary-container">Commission History</h3>
              <select className="bg-surface rounded-lg border border-outline-variant/50 font-body-sm text-body-sm text-on-surface-variant px-3 py-1.5">
            ...
            <div className="h-64 w-full flex items-end justify-between gap-2 px-4">
              {weeklyData.map((value, index) => (
                <div key={index} className="flex flex-col items-center flex-1">
                  <div
                    className="w-full bg-secondary-container rounded-t-lg transition-all"
                    style={{ height: `${(value / Math.max(...weeklyData)) * 100}%`, minHeight: '20px' }}
                  ></div>
                  <span className="font-data-md text-data-md text-on-surface mt-2">{value}</span>
                  <span className="font-label-caps text-label-caps text-on-surface-variant text-[10px]">Week {index + 1}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Referral List */}
          <div className="bg-surface-alt rounded-xl shadow-sm border border-outline-variant/30 overflow-hidden">
            <div className="p-6 border-b border-outline-variant/30 flex justify-between items-center bg-surface-container-low">
              ...
              <button className="text-secondary hover:text-secondary-fixed-dim transition-colors font-label-caps text-label-caps flex items-center gap-1">
                <span>View All</span>
