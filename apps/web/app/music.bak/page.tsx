...
const genres = [
  ...
  { id: '2', name: 'Highlife', icon: 'queue_music' },
...
export default function MusicPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-on-background font-body-md antialiased pb-32 md:pb-8">
      {/* TopAppBar */}
      <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-4 py-2 bg-surface border-b border-surface-secondary shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-surface-variant flex items-center justify-center overflow-hidden border border-surface-secondary">
            <img alt="User Profile" className="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuAxZALsq3LtoPthaLtklhy1dM1-Aebovw6D--bbMS8NWVByRkK8WUmiuPMBDBoz8KDkhWN4m2_T8oLVUK6iaB7XArmmbfoSZKzK28bHuUfQ1jsWEYI2hwLVfCL33RhPurUu9JadjrKAUI-3vOov-V1dbu1xblJo2v0YDCJOteNeAFYKNzGcX8dLeMpMe3e4P8cpDg5_ULQLZhffRGCx6plsWQb3H_rhXKn0mPjVwKPKIXI5wCZ1_bkRX3ClSD1EkoS8U2S_i8pu8Y5s" />
          </div>
      ...
      <main className="pt-[60px] max-w-container-max mx-auto md:px-gutter px-margin-mobile">
        ...
        <section className="mb-6 mt-2">
          <div className="relative w-full shadow-sm rounded-xl max-w-2xl mx-auto">
            <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline">search</span>
            <input
              className="w-full h-14 pl-12 pr-4 bg-surface-container-lowest border border-surface-secondary rounded-xl font-body-md text-body-md focus:outline-none focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors placeholder:text-text-muted shadow-[0px_4px_20px_rgba(13,27,53,0.05)]"
        ...
        <section className="mb-8">
          <div className="rounded-xl overflow-hidden bg-primary-container text-on-primary relative h-64 md:h-80 shadow-[0px_4px_20px_rgba(13,27,53,0.15)] group cursor-pointer border border-secondary-fixed/20">
            ...
            <div className="absolute bottom-0 left-0 p-6 w-full flex justify-between items-end">
              <div>
                <div className="inline-flex items-center gap-1 bg-secondary-fixed/10 border border-secondary-fixed rounded-full px-3 py-1 mb-3 backdrop-blur-sm">
                  <span className="font-wallet-display text-[10px] uppercase text-secondary-fixed tracking-wider">Exclusive Drop</span>
                </div>
                <h2 className="font-headline-xl text-headline-xl text-on-primary mb-1">Neon Dreams</h2>
                <p className="font-body-lg text-body-lg text-inverse-primary">The new album by Synthwave Collective</p>
              </div>
              <button className="w-14 h-14 bg-secondary-container text-on-secondary-container rounded-full flex items-center justify-center hover:bg-[#8B6914] transition-colors shadow-lg active:scale-95 shrink-0">
                <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1", fontSize: '32px' }}>play_arrow</span>
              </button>
            </div>
          </div>
        </section>
        ...
        <section className="mb-8">
          <div className="flex justify-between items-end mb-4">
            <h2 className="font-headline-md text-headline-md text-text-main">Trending Now</h2>
            <a className="font-body-sm text-body-sm text-secondary font-medium hover:underline" href="#">View All</a>
          </div>
          <div className="flex overflow-x-auto gap-4 pb-4 -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide">
            {trendingSongs.map((song) => (
              <div key={song.id} className="flex-none w-36 md:w-48 snap-start bg-surface-container-lowest rounded-lg border border-surface-secondary shadow-[0px_4px_20px_rgba(13,27,53,0.05)] p-3 group hover:-translate-y-1 transition-transform cursor-pointer">
                <div className="relative aspect-square rounded-md overflow-hidden mb-3 border border-surface-variant">
                  <div className="absolute inset-0 bg-primary-container/20 group-hover:bg-primary-container/50 transition-colors z-10 flex items-center justify-center opacity-0 group-hover:opacity-100 backdrop-blur-[2px]">
                    <button className="w-10 h-10 rounded-full bg-secondary-container text-on-secondary-container flex items-center justify-center shadow-md transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                      <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>play_arrow</span>
                    </button>
                  </div>
                  <img className="w-full h-full object-cover" src={song.cover} alt={song.title} />
                </div>
                <h3 className="font-body-md text-body-md font-semibold text-text-main truncate">{song.title}</h3>
                <p className="font-body-sm text-body-sm text-text-muted truncate">{song.artist}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Genres (Bento Grid) */}
        <section className="mb-8">
          <h2 className="font-headline-md text-headline-md text-text-main mb-4">Explore Genres</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {/* Afrobeats (Large Hero Tile) */}
            <div className="col-span-2 row-span-2 relative rounded-xl overflow-hidden shadow-sm border border-surface-secondary group cursor-pointer aspect-square md:aspect-auto md:h-full">
              <div className="absolute inset-0 bg-gradient-to-t from-primary-container/90 via-primary-container/40 to-transparent z-10"></div>
              ...
              <div className="absolute bottom-4 left-4 z-20">
                <h3 className="font-headline-lg text-headline-lg text-on-primary">Afrobeats</h3>
                <p className="font-body-sm text-body-sm text-inverse-primary mt-1">Trending global sounds</p>
              </div>
            </div>
            {/* Highlife */}
            <div className="relative rounded-xl overflow-hidden shadow-sm border border-surface-secondary group cursor-pointer aspect-square bg-surface-container-high">
              <div className="absolute inset-0 bg-primary-container/70 z-10 group-hover:bg-primary-container/80 transition-colors"></div>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                <span className="material-symbols-outlined text-secondary-fixed mb-2 text-3xl">queue_music</span>
                <h3 className="font-headline-md text-headline-md text-on-primary text-center">Highlife</h3>
              </div>
            </div>
            {/* Amapiano */}
            <div className="relative rounded-xl overflow-hidden shadow-sm border border-surface-secondary group cursor-pointer aspect-square bg-surface-container-high">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                <span className="material-symbols-outlined text-secondary-fixed mb-2 text-3xl">album</span>
                <h3 className="font-headline-md text-headline-md text-on-primary text-center">Amapiano</h3>
              </div>
            </div>
            {/* Hip Hop (Wide bottom) */}
            <div className="col-span-2 relative rounded-xl overflow-hidden shadow-sm border border-surface-secondary group cursor-pointer h-32 md:h-auto">
              <div className="absolute inset-0 bg-gradient-to-r from-primary-container/90 to-primary-container/30 z-10"></div>
              ...
              <div className="absolute left-6 top-1/2 -translate-y-1/2 z-20">
                <h3 className="font-headline-md text-headline-md text-on-primary">Hip Hop</h3>
                <p className="font-body-sm text-body-sm text-inverse-primary">Fresh beats &amp; flows</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mb-8">
          <div className="flex justify-between items-end mb-4">
            <h2 className="font-headline-md text-headline-md text-text-main">Top Artists</h2>
          </div>
          <div className="flex overflow-x-auto gap-4 pb-4 -mx-4 px-4 scrollbar-hide">
            {[
              ...
              <div key={index} className="flex-none w-24 md:w-32 snap-start flex flex-col items-center gap-2 cursor-pointer group">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-2 border-transparent group-hover:border-secondary-container transition-colors overflow-hidden shadow-sm">
                  <img className="w-full h-full object-cover" src={artist.image} alt={artist.name} />
                </div>
                <p className="font-body-sm text-body-sm font-semibold text-on-surface truncate w-full text-center">{artist.name}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <div className="fixed bottom-[72px] md:bottom-0 left-0 w-full z-40 bg-primary-container text-on-primary shadow-[0px_-8px_30px_rgba(13,27,53,0.15)] backdrop-blur-lg border-t border-primary-fixed-dim/20">
        {/* Progress Bar */}
        <div className="absolute top-0 left-0 w-full h-1 bg-surface-tint/30 cursor-pointer group/slider">
          <div className="h-full bg-secondary-container w-[45%] relative group-hover/slider:h-1.5 transition-all">
            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-secondary-fixed rounded-full shadow-md opacity-0 group-hover/slider:opacity-100 transition-opacity translate-x-1/2"></div>
          </div>
        </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-2 py-3 bg-primary-container shadow-[0px_-4px_20px_rgba(13,27,53,0.1)]">
        <Link href="/" className="flex flex-col items-center justify-center text-on-primary-fixed-variant px-3 py-1 hover:text-secondary-fixed transition-colors active:scale-95">
          <span className="material-symbols-outlined text-[24px]">home</span>
          <span className="font-label-caps text-label-caps mt-1">Home</span>
        </Link>
        <Link href="/tasks" className="flex flex-col items-center justify-center text-on-primary-fixed-variant px-3 py-1 hover:text-secondary-fixed transition-colors active:scale-95">
        ...
        <Link href="/marketplace" className="flex flex-col items-center justify-center text-on-primary-fixed-variant px-3 py-1 hover:text-secondary-fixed transition-colors active:scale-95">
          <span className="material-symbols-outlined text-[24px]">storefront</span>
          <span className="font-label-caps text-label-caps mt-1">Market</span>
        </Link>
        <Link href="/wallet" className="flex flex-col items-center justify-center text-on-primary-fixed-variant px-3 py-1 hover:text-secondary-fixed transition-colors active:scale-95">
